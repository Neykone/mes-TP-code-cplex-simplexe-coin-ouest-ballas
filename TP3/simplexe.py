...
def positivite(v):
    ...
    Args:
        v (np.ndarray): Le vecteur de coûts réduits (généralement la ligne des coûts 
                        du tableau simplexe, mais sans le dernier élément 'z').

    Returns:
        tuple: (booléen: True si tous >= 0, 
                float: valeur minimale du vecteur, 
...
def rapportmin(b, a):
    ...
    Args:
        b (np.ndarray): Vecteur des seconds membres (colonne 'b').
        a (np.ndarray): Colonne pivot (colonne 's' sélectionnée).
...
z_opt, x_opt, T_final = simplexe_primal(A_test, b_test, c_test)
